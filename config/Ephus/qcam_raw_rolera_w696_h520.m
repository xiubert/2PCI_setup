...
%  CA062712 -  replaced hard coded 1600 x 1200 pixel width and height to 696 x 520 (for rolera XR camera)
%  PC20260730 - configured for Rolera-XR firewire camera with 696 x 520 resolution w/ QCam board for interface w/ DAQ and driver: QCamInstaller-2.0.13.1 x64
%  
% SEE ALSO
%
...
% Last Modified by GUIDE v2.5 12-Nov-2012 16:17:04
...
out = {
       ...
       'width', 696, 'Class', 'numeric', 'Gui', 'width', 'Max', 696, 'Min', 1, 'Config', 7, ...
       'height', 520, 'Class', 'numeric', 'Gui', 'height', 'Max', 520, 'Min', 1, 'Config', 7, ...
       ...
       'namingScheme', 'XSG', 'class', 'char', 'Gui', 'namingScheme', ...
       'black', 0, 'Class', 'numeric', 'Gui', 'black', 'Min', 0, 'Config', 7, ...
       'white', 200, 'Class', 'numeric', 'Gui', 'white', 'Min', 0, 'Config', 7, ...
       'fig', [], ...
       'im', [], ...
       'debugOn', 0, 'Config', 7, ...
       'imageCounter', 1, ...   %%%%CTA added to duplicate grab video functionality from mapper in qcam
...
if ~isempty(imdata)
    ...
    if flipVertical
        imdata = flipud(imdata);%TO090910A - Hat tip to Charles Anderson for finding/fixing/reporting this. -- Tim O'Connor 9/9/10
    end
...
setLocalBatch(progmanager, hObject, ...
    'xOffset', 0, 'yOffset', 0, 'width', 696, 'height', 520, 'fullview', 1);
...
%%%%%%%%%%%%CTA adapting code from mapper to save images from qcam
function imageSave_Callback(hObject, eventdata, handles)

% mapper_userFcn_preGrabVideo(gcbo,[],guidata(gcbo))
...
[f, imageCounter] = getLocalBatch(progmanager, hObject,'videoFigure','imageCounter');

%from mapper, something to do with reusing the same videoImg window --CTA
...
try
    if xsg_getAutosave
        ...
        imdata = double(imdata);%TO050806C - Make sure it's a double.
        imdata = flipud(rot90(imdata)); %CA062812- the images use a different origin that\n matlab matrices, this rotates them so left on the image is left in the file
        imdata = flipud((imdata));
        imwrite(flipdim(imdata, 1) / max(max(imdata)), imageFileName, 'TIFF');
...
% --- Executes during object creation, after setting all properties.
function statusMessage_CreateFcn(hObject, eventdata, handles)
% hObject    handle to statusMessage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called


% --- If Enable == 'on', executes on mouse press in 5 pixel border.
% --- Otherwise, executes on mouse press in 5 pixel border or over statusMessage.
function statusMessage_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to statusMessage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
