...
xsgStartDirectory = 'C:\rig\DATA_TEMP';                 % Default root directory used by XSG program for saving data files. NOTE: This value can be overridden by a saved XSG configuration, which is often desirable.
% xsgStartDirectory = 'D:\Data\scientifica\'; 
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pockelsModulatorMin = -1; % Minimum value, in volts, to use when calibrating Pockels cell. Set this level somewhat below the expected value for minimum transmission. 
pockelsModulatorMax = 1; % Maximum value, in volts, to use when calibrating Pockels cell. Set this level somewhat above the expected value for maximum transmission.
...
xVideoImageSize = [];
yVideoImageSize = [];
% xVideoImageSize = [2233];
% yVideoImageSize = [1670];
...
% amp(1).serialNumber = [00830067];               %(MULTICLAMP 700B ONLY) Identifies serial number of amplifier device on which this channel is located
amp(1).serialNumber = [];
...
% amp(2).amplifierType = 'multi_clamp';              % One of {'multi_clamp' 'axopatch_200B'}. Identifies type of amplifier device.
% amp(2).scaledOutputBoardID = [1];        % Number identifying DAQmx board (e.g. 1 for 'Dev1') on which 'scaled output' signal from amplifier channel is input to Ephus
% amp(2).scaledOutputChannelID = [0];      % Number identifying DAQmx channel on board identified by 'scaledOutputBoardID' on which 'scaled output' signal from amplifier channelis input to Ephus
% amp(2).vComBoardID = [1];                % Number identifying DAQmx board (e.g. 2 for 'Dev2') on which command signal from Ephus is output to amplifier channel   
% amp(2).vComChannelID = [0];              % Number identifying DAQmx channel on board identified by 'vComBoardID' on which command signal from Ephus is output to amplifier channel   
% amp(2).amplifierChannelID = [2];         %(MULTICLAMP ONLY) Identifies which amplifier 'channel' on the amplifier device, e.g. 1 or 2 on Multiclamp amplifiers.
% amp(2).serialNumber = [00830067];               %(MULTICLAMP 700B ONLY) Identifies serial number of amplifier device on which this channel is located
% amp(2).comPortID = [];                  %(MULTICLAMP 700A ONLY) Identifies which serial port (e.g. 3 for COM3 port) is used to connect to amplifier device
% amp(2).axoBusID = [];                   %(MULTICLAMP 700A ONLY) Identifies which 'Axo Bus' is used for this amplifier channel
% amp(2).gainBoardID = [];                %(AXOPATCH 200B ONLY) Number identifying DAQmx board (e.g. 1 for 'Dev1') on which 'gain' telegraph signal from amplifier channel is input to Ephus
% amp(2).gainChannelID = [];              %(AXOPATCH 200B ONLY) Number identifying DAQmx channel on board identified by 'gainBoardID' on which 'gain' telegraph signal from amplifier channel is input to Ephus
% amp(2).modeBoardID = [];                %(AXOPATCH 200B ONLY) Number identifying DAQmx board (e.g. 1 for 'Dev1') on which 'mode' telegraph signal from amplifier channel is input to Ephus
% amp(2).modeChannelID = [];              %(AXOPATCH 200B ONLY) Number identifying DAQmx channel on board identified by 'modeBoardID' on which 'mode' telegraph signal from amplifier channel is input to Ephus
% amp(2).vHoldBoardID = [];               %(AXOPATCH 200B ONLY) Number identifying DAQmx board (e.g. 1 for 'Dev1') on which '10Vm' telegraph signal from amplifier channel is input to Ephus
% amp(2).vHoldChannelID = [];             %(AXOPATCH 200B ONLY) Number identifying DAQmx channel on board identified by 'vHoldBoardID' on which '10Vm' telegraph signal from amplifier channel is input to Ephus
...
acqChannelNames = {'sound output check', 'unused (AI17)'};                   % Cell array of descriptive names for analog input acquisition channels to configure, e.g. {'Lick Sensor' 'Nose Poke Sensor'}, 
...
% digStimChannelNames = {'Camera Digital'};               % Cell array of descriptive names for digital output stimulus channels to configure, e.g. {'Camera Trigger' 'LED Pulse'}
% digStimBoardIDs = [1];                   % A single number (e.g. 1 for 'Dev1') specifying DAQmx board for /all/ named digital stimulus channels; or, an array of numbers of length equal to 'digStimChannelNames' (e.g. [1 1 1 2 2] indicating 'Dev1' for first 3 channels, 'Dev2' for last 2 channels) identifying DAQmx board on which each of the named digital stimulus channels appears. 
% digStimPortIDs = [0];                    % A single number (e.g. 0 for Port 0 (P0)), or array of numbers of length equal to 'digStimChannelNames' (e.g. [0 0 0 1 1] indicating Port 0 (P0) for first 3 channels and Port 1 (P1) for for last 2 channels), identifying DAQmx board on which each of the named digital stimulus channels appears.
% digStimLineIDs =  [7];                   % Array of numbers, of length equal to 'digStimChannelNames', identifying DAQmx line number (e.g. 1 for P0.1, P1.1, etc) for each of the named digital stimulus channels (e.g. [0 1 2 0 1] indicating P#.0-2 for first 3 channels and P#.0-1 for last 2 channels, for case of multiple ports).
...
% sampleClockOrigin = 'dev2/ctr1';                 % Full DAQmx specification of single counter output channel on /one/ board (e.g. '/dev1/ctr0') on which the sample clock generated by Ephus appears. Ephus employs the default DAQmx routing of counter output channels to PFI output terminals (PFI12 for CTR0, PFI13 for CTR1; refer to DAQmx documentation for further information), Full DAQmx specification of counter output channel on /one/ board (e.g. '/dev1/ctr0') on which the sample clock is generated.
% sampleClockDestination = 'PFI6';            % A DAQmx PFI terminal name (e.g. 'PFI1') on which sample clock is input on /all/ of the boards used by Ephus
...
% clearInputBuffersOnStop was false, trying true to resolve long cam trace
% collection
clearInputBuffersOnStop = true;                % Logical (true/false) indicating, if true, that data buffers are cleared on stop command. Setting this to true may lead to loss of data, under certain circumstances, so it is generally set to false.
