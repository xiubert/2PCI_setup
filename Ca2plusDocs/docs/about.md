...
- **Herringbone artifact (Sutter rig)**: Diagonal stripes from ripple on the H10770PA-40 module's built-in high-voltage supply beating against the line rate. Normal for this PMT, not a fault. The ripple frequency follows the gain control voltage, so the artifact is periodic in gain — step the gain to reduce it. See [Hardware &rarr; Herringbone artifact](hardware_sutter.md#herringbone-artifact)
